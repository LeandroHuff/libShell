...
function getVersionStr()         { echo -n "${VERSION[0]}.${VERSION[1]}.${VERSION[2]}" ; }
function printVersion()          { echo -e "Version: ${WHITE}$(getVersionStr)${NC}" ; }
function getVersionNum()         { echo -n $(( ${VERSION[0]}*100 + ${VERSION[1]}*10 + ${VERSION[2]} )) ; }
...
function libStop() { unsetVars ; return 0 ; }
...
# init library and parse command line parameters.
function libStart()
{
	while [ -n "$1" ]
	do
		...
		-V|--version) printVersion   ; return 0 ;;
		-q|--quiet)   LEVEL=$QUIET   ; LOG=$(( (LOG - (LOG % 10)) + LEVEL)) ;;
		+q)           LEVEL=$NORMAL  ; LOG=$(( (LOG - (LOG % 10)) + LEVEL)) ;;
		-v|--verbose) LEVEL=$VERBOSE ; LOG=$(( (LOG - (LOG % 10)) + LEVEL)) ;;
		+v)           LEVEL=$NORMAL  ; LOG=$(( (LOG - (LOG % 10)) + LEVEL)) ;;
		-g|--debug)   DEBUG=true  ;;
		+g)           DEBUG=false ;;
		-t|--trace)	  TRACE=true  ;;
		+t)	          TRACE=false ;;
		-l|--log)
			if isArgValue $2 ; then
				...
				logF "Empty value for parameter -l|--log <0|1|2|3>"
				return 1
			fi
			;;
        -T|--timeout)
            ...
            ;;
        -*) logF "Invalid parameter ($1)."      ; return 1 ;;
        *) logF "Invalid parameter value ($1)." ; return 1 ;;
        esac
        shift
    done
    return 0
