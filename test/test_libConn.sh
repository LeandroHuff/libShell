...
################################################################################
# @file     test_libConn.sh
# @brief    Test and check libConn file.
# @parameter
...
function test_isConnected()
{
    local err=0
    local ret=0
    ping -t $1 -c $2 '8.8.8.8' > /dev/null 2>&1
    ret=$?
    if [ $ret -eq 0 ] && isConnected $1 $2
    then
        true
    elif [ $ret -ne 0 ] && ! isConnected $1 $2
    then
        true
    else
        false
...
do
    ...
    -t|--type)
        ...
        then
            shift
            if [ $1 -ge 0 ] && [ $1 -lt $maxTYPE ]
            then
                testTYPE=$1
            else
                logFail "Argument for -t|--type <0|1|2> is out of range."
                _exit 2
            fi
        else
            logFail "Empty or wrong argument for -t|--type <0|1|2>"
            _exit 3
        fi
        ;;
    -g) flagDEBUG=1 ;;
